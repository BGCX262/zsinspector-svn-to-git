...
"<tr>
	...
	<td ".(($reg_field->error_message <> '')?"class='error' title='$reg_field->error_message'":"").">
		<select id=\"$reg_field->field_name\" name=\"$reg_field->field_name\">
			<option value=\"\">Select $reg_field->display_name</option>";

	foreach($divisions as $division){
	//	print("'".$country_code."::".$division->country."::".$division->code."'<br/>");
		if(($country_code == "") || (isset($division->country) && $country_code == $division->country->code) || $country_code == $division->code){
